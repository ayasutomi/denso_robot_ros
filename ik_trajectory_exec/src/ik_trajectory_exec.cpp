...
IKTrajectoryExecutor::IKTrajectoryExecutor()
{

    //create a client function for the IK service
    ik_client = node.serviceClient<moveit_msgs::GetPositionIK>(ARM_IK_NAME, true);
    ...
    action_client = new TrajClient("vs060/arm_controller/follow_joint_trajectory", true);

    //wait for the action server to come up
    while (ros::ok() && !action_client->waitForServer(ros::Duration(5.0)))
    {
        ROS_INFO("Waiting for the follow_joint_trajectory action server to come up");
    }
...
bool IKTrajectoryExecutor::run_ik(geometry_msgs::PoseStamped pose, double start_angles[6],
                                  double solution[6], std::string link_name)
{
    ...
    ik_request.ik_request.timeout = ros::Duration(5.0);
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_1");
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_2");
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_3");
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_4");
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_5");
    ik_request.ik_request.robot_state.joint_state.name.push_back("joint_6");
...
void IKTrajectoryExecutor::get_current_joint_angles(double current_angles[6])
{
    int i;

    //get a single message from the topic 'r_arm_controller/state'
    sensor_msgs::JointState::ConstPtr state_msg =
...
bool IKTrajectoryExecutor::execute_joint_trajectory(std::vector<double *> joint_trajectory)
{
    int i, j;
...
bool IKTrajectoryExecutor::execute_cartesian_ik_trajectory(geometry_msgs::PoseStamped newposeStamped)
{

    // int trajectory_length = newposeStamped.poses.size();
    int i, j;

    //IK takes in Cartesian poses stamped with the frame they belong to
    geometry_msgs::PoseStamped stamped_pose;
    stamped_pose.header = newposeStamped.header;
